  ...
  <div id="blog" class="flex flex-col w-[20%]">
    ...
    <div id="hover">
      <el-button
        ...
        @click="addToCart(props.alldata.id)"
      ...
      <a :href="`http://localhost:5173/product/${props.alldata.id}`">
        <el-button
          type="warning"
          plain
          style="padding: 15px 50px; border-radius: 2px"
...
// Share
const share = () => {
  const textToCopy = `http://localhost:5173/product/${props.alldata.id}`;
...
isProductLiked.qonday = localStorage.getItem(props.alldata.name);
